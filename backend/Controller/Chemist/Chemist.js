...
exports.medicineRecord = async (req, res, next) => {
    ...
    (result) => {
      ...
      if (!result.access.includes(patientId)) {
        ...
        return res.status(400).json({
          success: false,
          message: `You do not have right to read the prescription for this user`,
        });
      }
    }
  );
  ...
  try {
    ...
    existingRecord.medicines = existingRecord.medicines; // Parse the existing medicines data
    existingRecord.medicineBill = existingRecord.medicineBill; // Parse the existing medicine bills data
  } catch (error) {
    console.log("Record does not exist. Creating a new one.", error);
    existingRecord = { medicines: [], medicineBill: [] };
  }
  console.log(
    "23",
    JSON.parse(JSON.parse(JSON.parse(existingRecord.medicines)))
  );
  console.log("24", existingRecord.medicineBill);
  // invoke code to update the medicalRecord with recordId
  console.log("25", medicineBill);
  let ex = JSON.parse(JSON.parse(JSON.parse(existingRecord.medicines)));
  if (existingRecord.medicineBill) {
    let bi = JSON.parse(JSON.parse(existingRecord.medicineBill));
    console.log(bi);
    newbi = [...bi, ...medicineBill];
  } else {
    newbi = [...medicineBill];
  }
  neww = [...ex, ...medicines];
  console.log(JSON.stringify(newbi));
  console.log(JSON.stringify(medicineBill));
  const args = [
    recordId,
