...
function Table({ tableName, tableData, columnNames, accessCheckbox, accessHandler, access, orgName }) {

    console.log(tableName, tableData, columnNames, accessCheckbox, accessHandler, access);
                                        ...
                                        <table className="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200">
                                                {tableData.map((item, index) => {
                                                    ...
                                                    console.log("if id: ", access.includes(Number(item[`${orgName}Id`])));
                                                    console.log("access: ", access.includes(Number(item[`${orgName}Id`])))
                                                    return (
                                                        <tr key={index}>
                                                            {
                                                                keys.map((key, i) => {
                                                            ...
                                                            {/* <td className="px-3 py-4 whitespace-nowrap">
                                                                <div className="flex">
                                                                    <div className="ml-4">
                                                                        <div className="text-sm text-left font-medium text-gray-900">
                                                                        <input type="checkbox" checked={access.includes(Number(item[`${orgName}Id`]))} onChange={(e) => accessHandler(e, item[`${orgName}Id`])} />
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </td> */}
